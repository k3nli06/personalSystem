...
public record PersonalDto(Long id, String name, String email, String password, DepartmentDto department, List<RoleDto> role, String workstation) {
    ...
    public static PersonalDto toDto(Personal personal) {
        ...
        return new PersonalDto(personal.getId(), personal.getName(), personal.getEmail(), null, DepartmentDto.toDto(personal.getDepartment()), rolesDto, personal.getWorkstation());
    }

    public static Personal toEntity(PersonalDto personalDto) {
        Set<Role> roles = personalDto.role.stream().map(
            r -> RoleDto.toEntity(r)
        ).collect(Collectors.toSet());
        
        var personal = new Personal(personalDto.id(), personalDto.name, personalDto.email, null, DepartmentDto.toEntity(personalDto.department), personalDto.workstation);
